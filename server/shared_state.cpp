    ...
    , db_root_(db_root)
{
    ...
    if (db) {
        // Создание таблицы Friends, если её нет
...
void shared_state::websocket_subscribe_to_symbols(websocket_session* session, std::string chatId) {
    // Проверка chatId и членства пользователя
...
void shared_state::searchUsersByName(websocket_session* session, std::string searchTerm) {
    ...
    obj["topic"] = 12;  // SearchUsersByName
...
void shared_state::deleteFriend(websocket_session* session, int friendId)
{
    ...
    if (rc == SQLITE_OK && sqlite3_step(stmt) == SQLITE_DONE && sqlite3_changes(session->db) > 0) {
        obj["status"] = "success";
    } else {
        obj["status"] = "error";
        obj["error"] = "Не удалось удалить друга";
    }
    sqlite3_finalize(stmt);
    boost::shared_ptr<std::string> ss = boost::make_shared<std::string>(boost::json::serialize(obj));
    session->send(ss);
...
void shared_state::inviteToChat(websocket_session* session, int chatId, std::vector<int> userId, int parentUser) {
    ...
    // Send notification to invited users and the inviter
    boost::shared_ptr<std::string> ss = boost::make_shared<std::string>(boost::json::serialize(obj));
    std::lock_guard<std::mutex> lock(mutex_);
...
void shared_state::sendMsg(websocket_session* session, std::string message) {
    ...
    // Дополнительная проверка существования chatid и userid
    std::string checkExistsSql = "SELECT 1 FROM Chat WHERE id = ?; SELECT 1 FROM Users WHERE id = ?";
    sqlite3_stmt* existsStmt = nullptr;
    rc = sqlite3_prepare_v2(session->db, checkExistsSql.c_str(), -1, &existsStmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Ошибка подготовки SQL (проверка существования): " << sqlite3_errmsg(session->db) << "\n";
        boost::json::object error;
        error["topic"] = 3;
        error["error"] = "Ошибка базы данных";
        session->send(boost::make_shared<std::string>(boost::json::serialize(error)));
    ...
    sqlite3_bind_int(existsStmt, 1, std::stoi(chatId));
    if (sqlite3_step(existsStmt) != SQLITE_ROW) {
        std::cerr << "Чат с ID " << chatId << " не существует\n";
        boost::json::object error;
        error["topic"] = 3;
        error["error"] = "Чат не существует";
        session->send(boost::make_shared<std::string>(boost::json::serialize(error)));
        sqlite3_finalize(existsStmt);
        return;
    }
    sqlite3_reset(existsStmt);
    sqlite3_bind_int(existsStmt, 1, userId);
    if (sqlite3_step(existsStmt) != SQLITE_ROW) {
        std::cerr << "Пользователь с ID " << userId << " не существует\n";
        boost::json::object error;
        error["topic"] = 3;
...
void shared_state::parse(std::string msg, websocket_session* session)
{
    try {
        ...
        case parser::MsgType::AddFriend:
        {
            ...
            notify["topic"] = 17; // NewFriendRequest
            notify["friend_id"] = userId;
...
void shared_state::acceptFriendRequest(websocket_session* session, int userId, int friendId) {
    ...
    if (sqlite3_step(stmt) == SQLITE_DONE && sqlite3_changes(session->db) > 0) {
        ...
        boost::json::object response;
        response["topic"] = 15; // Новый тип для принятия
...
void shared_state::getFriendsList(websocket_session* session, int userId) {
    ...
    // Загружаем список заявок в друзья
