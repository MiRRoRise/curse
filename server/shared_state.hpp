...
public:
    ...
    // Методы для работы с клиентами и чатами
    ...
    void createChat(websocket_session* session, std::string chatName, std::vector<std::string> invited, bool isVoiceChat = false); // Обновлено
    ...
    void deleteFriend(websocket_session* session, int friendId); // Добавлено
    void logout(websocket_session* session);                    // Добавлено
    void deleteVoiceChat(websocket_session* session, int chatId); // Добавлено
