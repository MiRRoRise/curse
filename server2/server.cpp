...
RTPServer::RTPServer(unsigned short port)
    : socket_(io_context_, udp::endpoint(udp::v4(), port)),
      cleanup_timer_(io_context_), // Инициализируем cleanup_timer_ с io_context_
      strand_(boost::asio::make_strand(io_context_)) // Инициализируем strand_ с io_context_
{
    socket_.non_blocking(true);
    startReceive();
    startCleanupTimer(); // Запускаем таймер очистки
}
